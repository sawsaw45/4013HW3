...
function selectDetailsByWorkout($wid) {
    try {
        ...
        $stmt = $conn->prepare("Select workoutdetailid, workoutid, workoutname, username, repetitions, sets, wd.weight, duration FROM Users u JOIN Workouts w ON u.UserID = w.UserID JOIN WorkoutDetails wd ON w.WorkoutID = wd.WorkoutID WHERE wd.workoutid = ?; ");
